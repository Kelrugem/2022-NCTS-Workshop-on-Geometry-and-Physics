...
\newcommand\oversortoftilde[1]{\ThisStyle{%
...
\section{Infinitesimal curved Yang-Mills-Higgs gauge theories}
...
\begin{frame}{Classical situation}
...
\mright)
\eas
for all $\nu \in \Gamma(\mathcal{g})$ and $p \in \mathcal{P}$, where $\mathcal{g}$ is the LAB\footnote{Lie algebra bundle} of $\mathcal{G}$.
\end{definition}
\end{frame}
...
\mathrm{D}_pr_\sigma(X)
	...
	+ \mleft.{\oversortoftilde{\mleft( \mu_{\mathcal{G}}\mright)_{g} (Y)}}\mright|_{p \cdot g}
\eas
\end{proposition}
\pause
\begin{definition}[Modified right-pushforward]
\bas
\mathcal{r}_{g*}(X)
&\coloneqq
...
\subsection{... as horizontal distribution}
{
\setbeamertemplate{footline}{}
...
\subsection{... as connection 1-form}
{
\setbeamertemplate{footline}{}
\begin{frame}{Field of gauge bosons}
